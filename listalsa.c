/****************************************************************************/
/*																			*/
/*	Use tab spacing of 4 to view this program correctly.					*/
/*																			*/
/****************************************************************************/
...
void main()
{
	...
	while (1)
	{
		...
		/****************************************************************/
		for ( count = 1; count <= numControls; count++ )
		{
			snd_ctl_elem_info_set_numid( info, count );

			// Is the control valid?
			errNum = snd_ctl_elem_info( ctl, info );
			if ( errNum < 0 )
			{
				printf( "Control is not valid.\n" );
			}
			printf( "Control is valid.\n" );

//			printf( "Numid = %s.\n", snd_hctl_elem_get_numid( elem ));
			elem = snd_hctl_find_elem( hctl, id );
			if ( elem == NULL ) printf ("Element not found.\n" );
