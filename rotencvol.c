...
{
    .cardName = "hw:0",     // 'default' also works but only for 1st card.
...
void encoderPulse()
{
    ...
    // Read values at pins.
    unsigned int pinA = digitalRead( setParams.wiringPiPinA );
    unsigned int pinB = digitalRead( setParams.wiringPiPinB );
...
static char doc[] = "A program to control volume using a rotary encoder.";
...
int getWiringPiNum( int gpio )
{
    int loop;
    int wiringPiNum;
    const int elems = 23; // No of GPIO pins in array.
    int pins[2][23] =
    {
